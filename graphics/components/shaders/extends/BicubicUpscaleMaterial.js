import { ShaderMaterial, Vector2, NoBlending } from "three";
...
void main() {
    vec2 uv = vUv;
    // vec4 res = textureBicubic(uTexture, uv, .5);
    vec4 res = texture2D(uTexture, uv);
...
class BicubicUpscaleMaterial extends ShaderMaterial {
  constructor() {
    super({
      uniforms: {
        uTexture: {
